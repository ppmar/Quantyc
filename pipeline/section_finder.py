...
def find_relevant_pages(
    ...
    if results:
        ...
        logger.info(
            ...
            len(results), section_type, pdf_path, page_nums,
        )
    else:
        logger.warning("No pages scored >= %d for '%s' in %s", min_score, section_type, pdf_path)
...
def find_relevant_pages_multi(
    ...
    if results:
        ...
        logger.info(
            "Found %d relevant pages for %s in %s (pages: %s)",
            len(results), section_types, pdf_path, page_nums,
        )
